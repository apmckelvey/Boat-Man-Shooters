...
float fbm(vec2 p) {
    ...
    float a = 0.5;
    float f = 1.0;
    // Increased octaves from 4 to 6 for more detail
    for(int i=0;i<6;i++){
        v += a * noise(p * f);
        f *= 2.0;
        a *= 0.5;
...
float wakePattern(vec2 p, vec2 boatPos, float boatRot, float speed, float t) {
    ...
    // Multiple ripple layers for better detail
...
void main() {
    ...
    // Increased posterization levels for smoother gradients
    ...
    else if (waves < 0.35) waterColor = mix(deepWater, darkWater, (waves - 0.2) / 0.15);
    else if (waves < 0.5) waterColor = mix(darkWater, midWater, (waves - 0.35) / 0.15);
    else if (waves < 0.65) waterColor = mix(midWater, lightWater, (waves - 0.5) / 0.15);
    else if (waves < 0.78) waterColor = mix(lightWater, brightWater, (waves - 0.65) / 0.13);
    else waterColor = mix(brightWater, foamColor, (waves - 0.78) / 0.22);

    // Increased posterization for smoother color bands
    waterColor = posterizeColor(waterColor, 12.0);
